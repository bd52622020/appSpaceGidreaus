...
ratings_columns = ["user id", "item id", "rating", "timestamp"]  #original "item id" renamed as "movie id"
...
def parse_input_2(fields):
#    fields = line.split("|")
    if fields[2] == "":
        return (fields[0],  fields[1], None)
    else:
        return (fields[0],  fields[1], datetime.strptime(fields[2], '%d-%b-%Y'))
...
def get_genres_sums(df, cols, num):
...
if __name__ == "__main__":
    ...
    results_5 = [(r[2], top_rated_movies_genres[ids.index(str(r[0]))]) for r in results_1]
    ### Q6
    ...
    ### Q8
    results_8 = users_df.select('zip code').groupBy('zip code').count().orderBy('count', ascending=False).show(10)
    ### Q9
    ...
    results_9 = movies_df.select('movie id', 'movie title').filter(movies_df["movie id"] == movies_id).select('movie title').collect()
    ### Q10
    ...
    results_10 = movies_df.select(["movie id", "movie title"]).filter(movies_df["movie id"].isin(movies_ids)).collect()
